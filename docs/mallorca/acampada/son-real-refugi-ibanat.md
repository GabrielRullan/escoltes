# ⛺ Son Real (Refugi i Cases)

Refugi a la finca arqueològica i litoral de Son Real.

---

## 🗺️ Mapa Interactiu (OpenStreetMap)

<iframe width="100%" height="350" frameborder="0" scrolling="no" marginheight="0" marginwidth="0" src="https://www.openstreetmap.org/export/embed.html?bbox=3.175%2C39.7405%2C3.205%2C39.7615&amp;layer=mapnik&amp;marker=39.751%2C3.19" style="border: 1px solid #ddd; border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.1);"></iframe>
<p style="margin-top: 4px; margin-bottom: 16px;"><small><a href="https://www.openstreetmap.org/?mlat=39.751&amp;mlon=3.19#map=15/39.751/3.19" target="_blank">🗺️ Obrir mapa complet a OpenStreetMap</a></small></p>

---

## 📋 Informació i Serveis de la instal·lació

| Característica | Detall |
| :--- | :--- |
| **Titularitat** | **IBANAT (Govern de les Illes Balears)** |
| **Municipi** | **Santa Margalida / Muro** |
| **Capacitat Màxima** | **12 persones** |
| **Antelació Permís** | **Reserva online IBANAT** |
| **Contacte / Reserves** | **971 17 76 52 (IBANAT)** |
| **Accés d'Emergència** | **Entrada finca Son Real Ma-12** |

---

## 🛠️ Infraestructura disponible
- **Serveis:** Habitacions, Aigua potable, Lavabos, Centre d'interpretació
- **Normativa de Foc:** Prohibit fer foc.

> [!IMPORTANT]
> Recordeu que entre l'1 de maig i el 15 d'octubre està **prohibit qualsevol tipus de foc** a l'aire lliure a totes les zones boscoses de Mallorca.

---

## 📍 Relació Realista de Rutes i Excursions

### 🥾 Rutes directament accessibles a peu des del refugi (<= 2.0 km)
| Ruta | Distància Ruta | Dificultat | Distància al Refugi | Enllaç |
| :--- | :---: | :---: | :---: | :--- |
| **Ruta Litoral de la Finca Pública de Son Real** | 8.5 km | Molt Fàcil | **0.00 km** (🟢 A peu) | [Veure Ruta](../rutes/son-real-can-picafort.md) |
### 🚌 Excursions a la zona que requereixen transport (> 2.0 km)
| Ruta | Distància Ruta | Dificultat | Distància des del refugi | Enllaç |
| :--- | :---: | :---: | :---: | :--- |
| **Parc Natural de s'Albufera de Mallorca** | 6.2 km | Molt Fàcil | **7.7 km** | [Veure Ruta](../rutes/parc-natural-albufera-mallorca.md) |
| **Excursió de Betlem a Platja des Caló (Artà)** | 7.0 km | Molt Fàcil | **11.8 km** | [Veure Ruta](../rutes/betlem-a-playa-es-calo.md) |

### ⚜️ Agrupaments Escoltes Més Propers
| Agrupament / Casal | Municipi | Distància | Enllaç |
| :--- | :--- | :---: | :--- |
| **AEG Sa Marjal** | Sa Pobla | **14.3 km** | [Veure Casal](../agrupaments/aeg-sa-marjal.md) |
| **AEG Pedra Viva** | Binissalem | **30.5 km** | [Veure Casal](../agrupaments/aeg-pedra-viva.md) |
