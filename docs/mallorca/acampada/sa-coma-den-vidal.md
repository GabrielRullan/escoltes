# ⛺ Refugi de Sa Coma d'en Vidal

Refugi del GR-221 a la Tramuntana sud amb opció d'acampada en tendes.

---

## 🗺️ Mapa Interactiu (OpenStreetMap)

<iframe width="100%" height="350" frameborder="0" scrolling="no" marginheight="0" marginwidth="0" src="https://www.openstreetmap.org/export/embed.html?bbox=2.44%2C39.6275%2C2.47%2C39.6485&amp;layer=mapnik&amp;marker=39.638%2C2.455" style="border: 1px solid #ddd; border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.1);"></iframe>
<p style="margin-top: 4px; margin-bottom: 16px;"><small><a href="https://www.openstreetmap.org/?mlat=39.638&amp;mlon=2.455#map=15/39.638/2.455" target="_blank">🗺️ Obrir mapa complet a OpenStreetMap</a></small></p>

---

## 📋 Informació i Serveis de la instal·lació

| Característica | Detall |
| :--- | :--- |
| **Titularitat** | **Consell de Mallorca (Xarxa GR-221)** |
| **Municipi** | **Estellencs** |
| **Capacitat Màxima** | **24 persones** |
| **Antelació Permís** | **Reserva online via Consell de Mallorca** |
| **Contacte / Reserves** | **971 18 20 27 (Consell)** |
| **Accés d'Emergència** | **Camí des de la carretera Estellencs Ma-10** |

---

## 🛠️ Infraestructura disponible
- **Serveis:** Aigua potable, Habitacions, Cuina, Lavabos
- **Normativa de Foc:** Prohibit fer foc.

> [!IMPORTANT]
> Recordeu que entre l'1 de maig i el 15 d'octubre està **prohibit qualsevol tipus de foc** a l'aire lliure a totes les zones boscoses de Mallorca.

---

## 📍 Relació Realista de Rutes i Excursions

### 🥾 Rutes directament accessibles a peu des del refugi (<= 2.0 km)
| Ruta | Distància Ruta | Dificultat | Distància al Refugi | Enllaç |
| :--- | :---: | :---: | :---: | :--- |
| **GR-221 Etapa 3: Estellencs a Esporles** | 15.2 km | Moderada | **0.00 km** (🟢 A peu) | [Veure Ruta](../rutes/gr221-etapa-3-estellencs-esporles.md) |
### 🚌 Excursions a la zona que requereixen transport (> 2.0 km)
| Ruta | Distància Ruta | Dificultat | Distància des del refugi | Enllaç |
| :--- | :---: | :---: | :---: | :--- |
| **Puig de Galatzó (des de la Font des Pi)** | 11.2 km | Exigent | **2.7 km** | [Veure Ruta](../rutes/puig-de-galatzo-font-des-pi.md) |
| **Muela de s'Esclop (des de sa Vinya de Galatzó)** | 12.8 km | Exigent | **4.5 km** | [Veure Ruta](../rutes/muela-de-sesclop.md) |
| **Finca Pública de Planícia (Banyalbufar)** | 9.2 km | Moderada | **5.8 km** | [Veure Ruta](../rutes/finca-publica-planicia-banyalbufar.md) |

### ⚜️ Agrupaments Escoltes Més Propers
| Agrupament / Casal | Municipi | Distància | Enllaç |
| :--- | :--- | :---: | :--- |
| **AEG Son Sardina** | Palma | **17.1 km** | [Veure Casal](../agrupaments/aeg-son-sardina.md) |
| **AEG Reina Constança de Mallorca** | Palma | **17.5 km** | [Veure Casal](../agrupaments/aeg-reina-constanca.md) |
