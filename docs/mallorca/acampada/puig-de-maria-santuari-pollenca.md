# ⛺ Santuari del Puig de Maria

Refugi i hospederia al cim del Puig de Maria de Pollença.

---

## 🗺️ Mapa Interactiu (OpenStreetMap)

<iframe width="100%" height="350" frameborder="0" scrolling="no" marginheight="0" marginwidth="0" src="https://www.openstreetmap.org/export/embed.html?bbox=3.0029999999999997%2C39.863%2C3.033%2C39.884&amp;layer=mapnik&amp;marker=39.8735%2C3.018" style="border: 1px solid #ddd; border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.1);"></iframe>
<p style="margin-top: 4px; margin-bottom: 16px;"><small><a href="https://www.openstreetmap.org/?mlat=39.8735&amp;mlon=3.018#map=15/39.8735/3.018" target="_blank">🗺️ Obrir mapa complet a OpenStreetMap</a></small></p>

---

## 📋 Informació i Serveis de la instal·lació

| Característica | Detall |
| :--- | :--- |
| **Titularitat** | **Ajuntament de Pollença / Obreria** |
| **Municipi** | **Pollença** |
| **Capacitat Màxima** | **25 persones** |
| **Antelació Permís** | **Instància registrada a l'Ajuntament de Pollença** |
| **Contacte / Reserves** | **971 18 41 32 (Aj. Pollença)** |
| **Accés d'Emergència** | **A peu des de Pollença (35 min)** |

---

## 🛠️ Infraestructura disponible
- **Serveis:** Hospederia, Menjador, Aigua potable, Vistes a la badia
- **Normativa de Foc:** Prohibit fer foc.

> [!IMPORTANT]
> Recordeu que entre l'1 de maig i el 15 d'octubre està **prohibit qualsevol tipus de foc** a l'aire lliure a totes les zones boscoses de Mallorca.

---

## 📍 Relació Realista de Rutes i Excursions

### 🥾 Rutes directament accessibles a peu des del refugi (<= 2.0 km)
| Ruta | Distància Ruta | Dificultat | Distància al Refugi | Enllaç |
| :--- | :---: | :---: | :---: | :--- |
| **Volta al Puig de Maria (Pollença)** | 4.5 km | Fàcil | **0.00 km** (🟢 A peu) | [Veure Ruta](../rutes/volta-puig-de-maria-pollenca.md) |
### 🚌 Excursions a la zona que requereixen transport (> 2.0 km)
| Ruta | Distància Ruta | Dificultat | Distància des del refugi | Enllaç |
| :--- | :---: | :---: | :---: | :--- |
| **Coves Blanques (Cala Sant Vicenç / Pollença)** | 5.4 km | Fàcil | **6.1 km** | [Veure Ruta](../rutes/coves-blanques-pollenca.md) |
| **Camí Vell de Pollença a Lluc (per la Font del Tomir)** | 12.5 km | Moderada | **6.7 km** | [Veure Ruta](../rutes/monestir-de-lluc-cami-vell.md) |
| **Cala Boquer i Vall de Boquer (Pollença)** | 5.5 km | Fàcil | **7.1 km** | [Veure Ruta](../rutes/cala-boquer-pollenca.md) |

### ⚜️ Agrupaments Escoltes Més Propers
| Agrupament / Casal | Municipi | Distància | Enllaç |
| :--- | :--- | :---: | :--- |
| **AEG Sa Marjal** | Sa Pobla | **11.6 km** | [Veure Casal](../agrupaments/aeg-sa-marjal.md) |
| **AEG Pedra Viva** | Binissalem | **25.2 km** | [Veure Casal](../agrupaments/aeg-pedra-viva.md) |
